...
addEventHandler( "onClientPlayerEnterModShop", g_root, 
    function( vehicle, money, shopname )
        if getLocalPlayer() == source then
            ...
            upgradeChanged = { }
            newUpgrades = { }
            outputChatBox( "#FF7C7C[TALLER] #FFFFFF¡Bienvenido a el taller!, ¿Que le pondras al vehiculo?", 0,0,0, true )
            --2510.0168457031, -1547.8715820312, 23.424806594849
            emptyShoppingCart( )
...
addEventHandler( "onClientResourceStart", g_resRoot, function( )
        ...
        cartWnd = guiCreateWindow( 5, screenHeight - 320, 120, 150, "Piezas Adquiridas", false )
        ...
        costsLbl = guiCreateLabel( 10, 121, 110, 20, " Costo:  $", false, cartWnd )
        ...
        guiSetText( colorsWnd, "Selecciona el color" )
        ...
        resetBtn = guiCreateButton( 115, 512, 100, 25, "Resetear", false, colorsWnd )
        cancelBtn = guiCreateButton( 220, 512, 100, 25, "Cancelar", false, colorsWnd )
        ...
        shopGUI.buttons[ "Exit" ] = guiCreateButton( 10, buttonHeight * btns + 8, shopWidth, buttonHeight - 5, "Comprar/Salir", false, shopGUI.wnd )
        ...
        --guiSetPosition( upgradeGUI.wnd, guiGetPosition( shopGUI.wnd, false ))
        ...
        for i = 1, 17 do
            if buttonName == guiGetText( shopGUI.buttons[ i ] ) then
                if not hideSubToo and btnPressed ~= shopGUI.buttons[ i ] then
                    if guiGetVisible( colorsWnd ) == false then
                        guiSetVisible( colorsWnd, false )
                        colorButtonClicked( "hide" )
                    end
...
addEventHandler( "modShop_insufficiantFounds", g_root,
    function( )
        outputChatBox( "Dinero insuficiente.", 255, 255, 0 )
    end
)
