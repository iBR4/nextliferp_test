	...
	if #scoreboardColumns ~= 0 then
		...
		if scoreboardDrawn then
			...
			if serverInfo.server and showServerInfo then
				...
				dxDrawText( "¡ Bienvenido a Next Life !", topX+s(5), y, topX+scoreboardDimensions.width-s(10), y+dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont ), cWhite, fontscale(serverInfoFont, s(0.75)), serverInfoFont, "center", "top", false, false, drawOverGUI )
			...
			local textLength = dxGetTextWidth( "", fontscale(rmbFont, s(0.75)), rmbFont )

			local textHeight = dxGetFontHeight( fontscale(rmbFont, s(0.75)), rmbFont )

			dxDrawText( "", sX/2-(textLength/2), topY+scoreboardDimensions.height-textHeight-s(2), sX/2+(textLength/2), topY+scoreboardDimensions.height-s(2), cWhite, fontscale(serverInfoFont, s(0.75)), rmbFont, "left", "top", false, false, drawOverGUI )
