...
function warpBetweenInteriors( source )
	...
	local picka = source:getData("pickup.info")
	...
	if (intEntry[cols]) then
		...
		triggerClientEvent( source, "puerta", source )

		textDisplay = textCreateDisplay ( )
        textItem = textCreateTextItem ( "Ingresando a un interior...", 0.7, 0.9, 1.3, 255, 255, 255, 255, 2, "center", "center" )
		...
		textDisplay = textCreateDisplay ( )
        textItem = textCreateTextItem ( "Saliendo de un interior...", 0.7, 0.9, 1.3, 255, 255, 255, 255, 2, "center", "center" )
        textDisplayAddText ( textDisplay, textItem )
        for _, thePlayer in ipairs ( getElementsByType ( "player" ) ) do
            textDisplayAddObserver ( textDisplay, source )
        end
