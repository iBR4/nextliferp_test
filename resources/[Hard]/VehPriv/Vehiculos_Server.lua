...
function Vehicles_s:constructor(array, faction)
	...
	self.vehicles[plate][index].Veh:setLocked(false)

	self.vehicles[plate][index].Veh:setFrozen(true)

	self.vehicles[plate][index].Veh:setData('Maletero', maletero)

	self.vehicles[plate][index].Veh:setData("VehiculoPublico", faction)

	if bikes[array.ID] then

		self.vehicles[plate][index].Veh:setData("LockedVeh", false)
		...
		if permisos[getACLFromPlayer(player)] == true then
			...
			if tablet then
				...
				if isElement(vehicle.Veh) then
					...
					exports['Notificaciones']:setTextNoti(player, "#00FF00Vehículo de #FFFFFF"..faction.." #00FF00de #FFFFFFID ".. tonumber(placa).."#00FF00 encontrado y enviado.")

				end

			else		

				exports['Notificaciones']:setTextNoti(player, "Syntax: /traervehpub <clave> <id>")

				exports['Notificaciones']:setTextNoti(player, "Claves: Mary, ADAM, UNION, Louis, LINCON, TAC, TORH, DAVID, DIC, BUS, CHARL")
...
addCommandHandler("respawnvehpub",
	...
	function(player, cmd, faction, placa)
		...
		if permisos[getACLFromPlayer(player)] == true then
				...
				if tablet then
					...
					if isElement(vehicle.Veh) then
						...
						if isVehicleEmpty( vehicle.Veh ) then
							...
							vehicle.Veh:setLocked(vehicle.State, false)

							vehicle.Veh:setEngineState(false)

							setElementData(vehicle.Veh, "Fuel", 100)
							
							vehicle.Veh:setData('Maletero', maletero)

							vehicle.Veh:fixVehicle()

							exports['Notificaciones']:setTextNoti(player, "Vehiculo Respawneado")
					...
					--exports['[LS]Notificaciones']:setTextNoti(player, "Claves: Mary, ADAM, UNION,  Louis, LINCON, TAC, TORH, DAVID, DIC, BUS , CHARL")
					...
					for index,data in pairs(dd) do
						...
						if isVehicleEmpty( data.Veh ) then
							...
							if data.Veh:respawn() then
								...
								data.Veh:setLocked(data.State ,false)
			...
			outputChatBox("#FFFFFFTodos los #DDFF00vehículos públicos #FFFFFFhan sido respawneados.",source, 150, 50, 50, true)
...
addCommandHandler("naftavehs",
	...
	function(player, cmd)
		...
		if permisos[getACLFromPlayer(player)] == true then
			...
			outputChatBox("#FFFFFFTodos los #DDFF00vehículos públicos #FFFFFFse les ha llenado la gasolina.",source, 150, 50, 50, true)
			exports["Logs-Vehs"]:sendDiscordLogMessage(""..player:getData("Nombre:staff").." Lleno la gasolina de todos los vehiculos publicos")  
