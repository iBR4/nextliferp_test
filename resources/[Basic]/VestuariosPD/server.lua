...
addEvent("borrarBasuraPD", true)
function borrarPDBasura( player )
	if player then
		if takeAllWeapons( player ) then
			triggerEvent( "WestSide:MaleteroSave_DB", player, player)
		end
	end
end
addEventHandler("borrarBasuraPD", getRootElement(), borrarPDBasura)
