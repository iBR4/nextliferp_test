...
addEventHandler("onClientResourceStart",getResourceRootElement(getThisResource()),
	function()
		...
		bindKey ( "e", "down", "Siguiente arma", "1" )
		bindKey ( "q", "down", "Anterior arma", "-1" )
...
function toggleDriveby()
	...
	if equipedWeapon == 0 then
		...
		if prevw and nextw then
			...
			helpText:text( "Pulsa '"..prevw.."' o '"..nextw.."' para cambiar de arma" )
			fadeInHelp()
...
function limitedKeyPress (key,keyState,speed)
	if keyState == "down" then
		...
		if getPedTotalAmmo(getLocalPlayer()) == 1 then
			outputChatBox("¡No tienes balas! Recarga tu arma pulsando 'R' primero.", 255, 0, 0)
			return
		end
		shooting = true
