...
addCommandHandler("graffiti", function(p)
		if exports.familias:getPlayerFamily(p) then
			triggerClientEvent(p, "abrirPanelGraffiti", p)
	end
end)
...
addEventHandler("createGraffiti", getRootElement(),
	function (pixels, data)
		...
		if graffitiImage then
			...
			exports["Logs-OC"]:sendDiscordLogMessage("hizo un graffiti.", player)
		end
	end
)
